...
<html>
    ...
    <body>
        <h1>Dar de Alta Departamento</h1>
        <?php
        /**
          @author: Christian Muñiz de la Huerga
          @since: 12/11/2018
          Comentarios: permite volver a dar de alta un departamento.
         */
        ...
        try {
            ...
            $bajaLogica = $registro->bajaLogica;
            if (isset($_POST['Aceptar'])) { //Si se pulsa el botón submit se realizan las siguientes intrucciones.
                $consulta = $baseDeDatos->prepare("update Departamento set bajaLogica=null where CodDepartamento=:codigo"); //Prepara la consulta con dos incógnitas.
                ...
                <form action="<?php echo $_SERVER['PHP_SELF'] . '?CodDepartamento=' . $_GET['CodDepartamento']; ?>" method="post">
                    <div>
                        <table>
                            ...
                            <tr>
                                <td>Fecha de Baja</td>
                                <td><input type="text" name="bajaLogica" value="<?php echo $bajaLogica; ?>" disabled></td>
                            </tr>
                            <tr>
