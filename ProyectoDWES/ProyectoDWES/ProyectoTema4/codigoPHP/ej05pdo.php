...
<html>
    ...
    <body>
        ...
        /**
          ...
          @since: 06/11/2018
          Comentarios: en este programa se da de alta nuevos departamentos, todos o ninguno, en la base de datos con una transacción.
         */
        ...
        while ($num <= 3) { //Recorre las filas del array mientras ésta sea menor o igual a 4.
            ...
            $aFormulario[$numDep]['codDepartamento'] = null; //Asigna el valor null a la posición donde apunta el puntero del array.
            $aErrores[$numDep]['descDepartamento'] = null; //Asigna el valor null a la posición donde apunta el puntero del array.
            $aFormulario[$numDep]['descDepartamento'] = null; //Asigna el valor null a la posición donde apunta el puntero del array. 
        ...
        if ($entradaOK) { //Si la entrada de datos es correcta se imprimen por pantalla los campos.
            ...
            for ($numInsert = 1; $numInsert < $numero; $numInsert++) {
                $aFormulario[$numInsert]['codDepartamento'] = strtoupper($_POST['codDepartamento' . $numInsert]); //Recoge el valor de cada campo nombre ya validado.
                $aFormulario[$numInsert]['descDepartamento'] = $_POST['descDepartamento' . $numInsert]; //Recoge el valor de cada campo fecha ya validado.
            }
            try {
                ...
                for($numInsert=1;$numInsert<$numero;$numInsert++){ //Recorre todos los departamentos que van a insertarse.
                    $codigo=$aFormulario[$numInsert]['codDepartamento']; //Guarda el valor de cada código en una variable temporal para pasarle al insert.
                    $descripcion=$aFormulario[$numInsert]['descDepartamento']; //Guarda el valor de cada descripción en una variable temporal para pasarle al insert.
                    ...
                    if(!$consulta->execute()){ //Ejecuta el query y evalúa si ha introducido la tupla.
